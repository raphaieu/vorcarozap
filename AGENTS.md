...
- Métricas de rede contam somente claims públicos ativos com `metric_eligible = true`; contexto e correções podem ser públicos sem inflar vínculos.
...
- Moderação tem como alvo um `claim` ou um `evidence_source`, nunca a `source` global.
- Verificação de acessibilidade usa GET seguro e limitado; não usar HEAD nem transformar o verificador em crawler.
...
- Sem meta de cobertura. Testes table-driven de decisão de publicação, bloqueio por fingerprint rejeitado e métricas excluindo estados não públicos ou `metric_eligible = false` são obrigatórios. Execute `go fmt`, `go vet`, `go build` e smoke checklist.
