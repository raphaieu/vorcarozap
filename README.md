...
- **Fase 6 — Deploy MVP e validação econômica:**
  ...
  - **VZ-019 — Gate final de testes determinísticos de publicação/quarentena, bloqueio por fingerprint rejeitado e métricas por estado/elegibilidade concluído.** Suítes table-driven rigorosas e 100% determinísticas em SQLite isolado (`t.TempDir`) cobrindo:
    - Decisão de publicação/quarentena por Grau (A, B, C com/sem limites e linguagem cautelar, D e E com fail-closed incondicional);
    - Bloqueio estrutural de candidatos novos ou duplicados com fingerprint de claims previamente rejeitados por moderação humana;
    - Transações de moderação com controle otimista de concorrência (OCC), quarentena atômica por perda do último suporte, garantia de não-republicação automática em restaurações e auditoria imutável XOR em `moderation_decisions`;
    - Cálculo de métricas públicas com imunidade a divisão por zero, exclusão estrita de registros não-públicos ou com `metric_eligible = false`, e recálculo instantâneo após moderação;
    - Exportação XLSX estritamente isolada à fronteira pública, sem vazamento de evidências ou fontes rejeitadas, preservação de claims de contexto com flag "Não", e sanitização contra injeção de fórmulas (`=`, `+`, `-`, `@`);
    - Fronteira pública SSR e área `/admin` com cabeçalhos rigorosos de cache (`Cache-Control`, `Vary: Authorization`) e proteção CSRF estrita.

- **Fase 7 — Maturidade e evolução pós-MVP:**
  - **VZ-022 — Referenciação aprofundada de laudos periciais e peças documentais concluído.** Normalização determinística e sanitização estrita de localizadores documentais (`normalize.Locator`, `normalize.SafeLocator`, en-dash em intervalos numéricos `Págs. 42–44`, padronização em português para páginas, folhas, figuras, tabelas, anexos, itens, artigos e parágrafos), enriquecimento de tipos canônicos de fonte (`court_document`, `police_report`, `official_statement`, `article`, `interview`, `social_media`) com identificação visual de documentos oficiais primários (`source-type-primary`), segregação semântica e visual entre citação literal (`.source-excerpt`) e localizador cirúrgico (`.source-locator-box`/`.source-locator-chip`) nas visualizações públicas (`/pessoas/{slug}`) e administrativas (`/admin/evidencias/{id}`, `/admin/claims/{id}`, `/admin/candidatos/{id}`, `/admin/fontes`), mantendo integralmente o schema relacional e as garantias de integridade editorial sem criar novas tabelas, conforme [ADR-012](docs/adr/ADR-012-navegacao-documental-e-referencias-a-acervos-externos.md).
  - **VZ-023 — Avaliação técnica e Viewer SSR de documentos e sequências contextuais concluído.** Avaliação técnica e jurídica formal ([ADR-022](docs/adr/ADR-022-avaliacao-tecnica-e-viewer-ssr-de-sequencias-contextuais.md)) rejeitando proxying de PDFs/HTML remotos no servidor (prevenindo DoS, SSRF, bypass de CSP e infração de direitos autorais); implementação de visualizador SSR de documentos e sequências contextuais (`/documentos/{id}` e `/admin/fontes/{id}`) com ordenação determinística natural por localizadores (`CompareLocators`), rastreabilidade de integridade técnica observada na checagem de fontes, links bidirecionais de contexto documental nas fichas de entidades, isolamento estrito de itens em quarentena/rejeitados nas rotas públicas e garantia de invalidação imediata de cache com cabeçalhos rigorosos.
  - **VZ-024 — Histórico público de alterações editoriais e trilha de auditoria concluído.** Implementação da trilha de transparência pública nas páginas de pessoas (`/pessoas/{slug}`) e no visualizador documental (`/documentos/{id}`), com segregação estrita entre a auditoria administrativa completa no `/admin` e a visão pública redigida (preservando o anonimato de operadores com rótulo institucional "Equipe Editorial", omitindo motivos internos, fingerprints e custos de LLM), filtragem de proposições reprovadas em quarentena, ordenação determinística e invalidação dinâmica imediata, conforme [ADR-023](docs/adr/ADR-023-historico-publico-de-alteracoes-editoriais-e-trilha-de-transparencia.md).
