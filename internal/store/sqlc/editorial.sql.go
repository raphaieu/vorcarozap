...
const listPublicEditorialHistoryByEntityID = `-- name: ListPublicEditorialHistoryByEntityID :many
SELECT
    d.id AS decision_id,
    d.claim_id,
...
WHERE (
    (d.claim_id IS NOT NULL AND (rc.subject_entity_id = ?1 OR rc.target_entity_id = ?1))
    OR
    (d.evidence_source_id IS NOT NULL AND (resc.subject_entity_id = ?1 OR resc.target_entity_id = ?1))
)
ORDER BY d.created_at DESC, d.id DESC
LIMIT ?2
`

type ListPublicEditorialHistoryByEntityIDParams struct {
	EntityID   string `json:"entity_id"`
	EventLimit int64  `json:"event_limit"`
}

type ListPublicEditorialHistoryByEntityIDRow struct {
	...
	AssociatedClaimID                string         `json:"associated_claim_id"`
	AssociatedClaimProposition       string         `json:"associated_claim_proposition"`
	AssociatedClaimGrade             string         `json:"associated_claim_grade"`
	AssociatedClaimStatus            string         `json:"associated_claim_status"`
	AssociatedClaimOrigin            string         `json:"associated_claim_origin"`
	AssociatedClaimQuarantineReasons string         `json:"associated_claim_quarantine_reasons"`
	TargetEvidenceSourceID           string         `json:"target_evidence_source_id"`
	TargetEvidenceSourceLocator      string         `json:"target_evidence_source_locator"`
	TargetEvidenceSourceRole         string         `json:"target_evidence_source_role"`
	TargetEvidenceSourceStatus       string         `json:"target_evidence_source_status"`
	TargetSourceID                   string         `json:"target_source_id"`
	TargetSourceTitle                string         `json:"target_source_title"`
	TargetSourcePublisher            string         `json:"target_source_publisher"`
	IsClaimCurrentlyPublic           int64          `json:"is_claim_currently_public"`
	HadPriorApproval                 int64          `json:"had_prior_approval"`
}

func (q *Queries) ListPublicEditorialHistoryByEntityID(ctx context.Context, arg ListPublicEditorialHistoryByEntityIDParams) ([]ListPublicEditorialHistoryByEntityIDRow, error) {
	rows, err := q.db.QueryContext(ctx, listPublicEditorialHistoryByEntityID, arg.EntityID, arg.EventLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListPublicEditorialHistoryByEntityIDRow
	for rows.Next() {
		var i ListPublicEditorialHistoryByEntityIDRow
		if err := rows.Scan(
			&i.DecisionID,
			&i.ClaimID,
			&i.EvidenceSourceID,
			...
			&i.TargetType,
			&i.AssociatedClaimID,
			&i.AssociatedClaimProposition,
			&i.AssociatedClaimGrade,
			&i.AssociatedClaimStatus,
			&i.AssociatedClaimOrigin,
			&i.AssociatedClaimQuarantineReasons,
			&i.TargetEvidenceSourceID,
			&i.TargetEvidenceSourceLocator,
			&i.TargetEvidenceSourceRole,
			&i.TargetEvidenceSourceStatus,
			&i.TargetSourceID,
			&i.TargetSourceTitle,
			&i.TargetSourcePublisher,
			&i.IsClaimCurrentlyPublic,
			&i.HadPriorApproval,
		); err != nil {
			return nil, err
		}
...
const listPublicEditorialHistoryBySourceID = `-- name: ListPublicEditorialHistoryBySourceID :many
SELECT
    d.id AS decision_id,
    d.claim_id,
    d.evidence_source_id,
...
LEFT JOIN sources s ON s.id = COALESCE(es.source_id, (
    ...
    WHERE ev2.claim_id = d.claim_id AND es2.source_id = ?1
    LIMIT 1
))
WHERE (
    (d.evidence_source_id IS NOT NULL AND es.source_id = ?1)
    OR
    (d.claim_id IS NOT NULL AND EXISTS (
        SELECT 1 FROM evidence ev3
        JOIN evidence_sources es3 ON es3.evidence_id = ev3.id
        WHERE ev3.claim_id = d.claim_id AND es3.source_id = ?1
    ))
)
ORDER BY d.created_at DESC, d.id DESC
LIMIT ?2
...
type ListPublicEditorialHistoryBySourceIDParams struct {
	SourceID   string `json:"source_id"`
	EventLimit int64  `json:"event_limit"`
}

type ListPublicEditorialHistoryBySourceIDRow struct {
	DecisionID                       string         `json:"decision_id"`
	ClaimID                          sql.NullString `json:"claim_id"`
	EvidenceSourceID                 sql.NullString `json:"evidence_source_id"`
	Action                           string         `json:"action"`
...
func (q *Queries) ListPublicEditorialHistoryBySourceID(ctx context.Context, arg ListPublicEditorialHistoryBySourceIDParams) ([]ListPublicEditorialHistoryBySourceIDRow, error) {
	rows, err := q.db.QueryContext(ctx, listPublicEditorialHistoryBySourceID, arg.SourceID, arg.EventLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListPublicEditorialHistoryBySourceIDRow
	for rows.Next() {
		var i ListPublicEditorialHistoryBySourceIDRow
		if err := rows.Scan(
			&i.DecisionID,
			&i.ClaimID,
			&i.EvidenceSourceID,
