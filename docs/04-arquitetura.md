...
O sistema mantém segregação estrita entre auditoria administrativa e transparência pública ([ADR-023](adr/ADR-023-historico-publico-de-alteracoes-editoriais-e-trilha-de-transparencia.md)):
- **Auditoria Administrativa Completa (`/admin`):** Registrada na tabela imutável `moderation_decisions`, preserva o operador autenticado (`actor`), justificativa técnica/editorial fundamentada (`reason`), fingerprint SHA-256 e integridade relacional XOR (`claim_id` ou `evidence_source_id`).
- **Histórico Público Redigido (`/pessoas/{slug}` e `/documentos/{id}`):** Apresenta ao leitor uma linha do tempo transparente e neutra das publicações, retiradas, restaurações e alterações de suporte documental. Aplica política estrita de privacidade: operadores são redigidos para `Equipe Editorial`, justificativas brutas viram resumos padronizados neutros, fingerprints são omitidos e alegações que nunca foram públicas ou foram desaprovadas não têm suas proposições reexpostas.
