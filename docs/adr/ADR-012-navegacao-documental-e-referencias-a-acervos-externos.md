...
Aprovado (Consolidado pelas entregas VZ-022 e VZ-023 — ver [ADR-022](ADR-022-avaliacao-tecnica-e-viewer-ssr-de-sequencias-contextuais.md))
...
1. **Integração inicial estritamente por referências documentais e links precisos:**
   A conexão do VorcaroZAP com acervos documentais e laudos periciais (incluindo o IPJ-A nº 3298613/2026 e reportagens associadas) dar-se-á pela referenciação canônica de fontes (`sources`), citações literais textuais (`evidence_sources.excerpt`) e localizadores cirúrgicos (`evidence_sources.locator`), tais como `Pág. 42, Fig. 12` ou URL pública do documento oficial.
2. **Aproveitamento das estruturas existentes sem alteração de schema:**
   O schema relacional consolidado (`00002_editorial_core.sql` e a visão `public_claims_view`) já contempla suporte completo a `sources`, `evidence_sources`, `locator`, `excerpt` e `role` (`supports`, `contradicts`, `contextualizes`). Não há criação de novas tabelas ou migrações adicionais.
3. **Diferimento de importador dedicado de mensagens e de visualizador tipo chat em tempo real:**
   O desenvolvimento de um importador específico de mensagens estruturadas (ex: conversas JSON/JSONL) e de proxy reverso de arquivos remotos fica expressamente rejeitado por razões de segurança e direitos autorais. A navegação dá-se via visualizador SSR seguro de documentos e sequências contextuais ([ADR-022](ADR-022-avaliacao-tecnica-e-viewer-ssr-de-sequencias-contextuais.md)).
4. **Natureza secundária das transcrições e visualizadores:**
   Toda transcrição textual, leitura óptica ou visualização reproduzida em tela é classificada como representação secundária. O VorcaroZAP mantém visível e distinguível o documento oficial de origem, o número do laudo/processo, a página/figura original e as eventuais limitações de legibilidade relatadas pelos peritos ou curadores.
...
8. **Conformidade de direitos autorais e licenças:**
   Nenhum código-fonte ou arquivo de dados de repositórios terceiros sem autorização formal é incorporado à base de código do VorcaroZAP.
9. **Manutenção estrita da stack:**
   Nenhuma dependência externa, serviço em tempo real ou biblioteca SPA é adicionada.

## Alternativas consideradas

- **Incorporação direta do frontend e base do MasterWhats:** Rejeitada por violar a ausência de licença identificada, introduzir dependência de Node/Vite no runtime e desviar recursos do núcleo editorial.
- **Criação de tabelas relacionais para mensagens (`messages`, `chats`):** Rejeitada por introduzir complexidade precoce de schema. O campo `locator` e o visualizador SSR contextual atendem perfeitamente à necessidade de localização e sequenciamento pontual.
...
### Positivas
- Preserva a integridade da arquitetura monólito Go + SQLite + SSR.
...
- Viabiliza a referenciação cirúrgica e sequenciamento contextual de peças periciais através do schema relacional atual (`locator` e `excerpt`).

### Negativas e Limitações
- Não entrega interface em balões de diálogo estilo WhatsApp nem download direto de PDFs pesados da PF.
- O leitor navega pela sequência contextual estruturada e acessa o documento integral externo via link seguro.
